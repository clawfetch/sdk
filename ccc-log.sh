...
if [ -z "$CCC_API_KEY" ]; then
  # Try secrets file first (most reliable)
  SECRETS_FILE="$HOME/.openclaw/workspace/.secrets/ccc-api-key.txt"
  if [ -f "$SECRETS_FILE" ]; then
    CCC_API_KEY=$(cat "$SECRETS_FILE")
  else
    # Parse apiKeys section from local.yaml — use awk to avoid matching keyFile: line
    CONFIG_FILE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/config/local.yaml"
    if [ -f "$CONFIG_FILE" ]; then
      CCC_API_KEY=$(awk '/^[[:space:]]*apiKeys:/,/^[^[:space:]]/' "$CONFIG_FILE" | grep '^\s*- key:' | head -1 | sed 's/.*key: *"//' | sed 's/".*//')
    fi
  fi
fi
